#!/bin/bash
#
# ACP Server Startup Script
# 
# This script properly initializes the Poetry environment and starts the ACP server
# with all necessary environment variables loaded from .env
#
# Usage:
#   ./scripts/acp_server.sh [--port PORT] [--log-level LEVEL] [--reload]
#
# Options:
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  $0 --port 8002                  # Start on port 8002"
...
cleanup() {
    echo ""
    print_status "Shutting down ACP server gracefully..."
    exit 0
}
...
echo ""
echo "============================================================"
echo "  1Password Credential Broker - ACP Server Launcher"
echo "============================================================"
echo ""
...
# Step 7: Verify ACP server script exists
print_status "Verifying ACP server script..."
if [ ! -f "src/acp/run_acp.py" ]; then
    print_error "ACP server script not found: src/acp/run_acp.py"
    exit 1
fi
print_success "ACP server script found"
echo ""
...
echo ""
echo "API Endpoints:"
echo "  Agents List:      http://${HOST}:${PORT}/agents"
echo "  Run Agent:        http://${HOST}:${PORT}/run"
echo "  Session History:  http://${HOST}:${PORT}/sessions/{session_id}"
...
# Run the ACP server using Poetry
# Poetry automatically activates the virtual environment when using 'poetry run'
exec poetry run python src/acp/run_acp.py $CMD_ARGS
