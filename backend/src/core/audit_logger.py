...
class AuditLogger:
    ...
    def __init__(
        ...
        events_api_token: Optional[str] = None,
        max_retries: int = 3,
        ...
        self.events_api_enabled = bool(
            self.events_api_url and self.events_api_token
        )
        ...
        if not self.events_api_enabled:
            ...
            logger.info(
                f"1Password Events API enabled: {self.events_api_url}"
            )
    ...
    async def retry_failed_events(self) -> int:
        ...
        logger.info(
            f"Retrying {len(self.failed_events_queue)} failed events..."
        )
